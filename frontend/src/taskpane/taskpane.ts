...
async function runEmailSecurityScan(): Promise<void> {
  ...
  try {
    ...
    // --- 0. Identify the analyst running this scan ---------------------
    // This is what lets the admin dashboard attribute every scan to a
    // specific employee (per-user metrics, "click a user to see their scans").
    // Office.js exposes this without any extra login step.
    const scannedBy =
      safeGet(() => Office.context?.mailbox?.userProfile?.emailAddress) ?? "unknown_user";
    console.log("[PhishingShield] scanned_by (analyst identity):", scannedBy);
    ...
    const payload = {
      ...
      // Sent purely so the admin dashboard can show the exact raw source
      // behind a verdict. Never rendered back in the taskpane itself.
